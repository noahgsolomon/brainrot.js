...
export async function generateRapTranscript(
	topic: string,
	agentA: string,
	agentB: string
) {
...
export default async function rapTranscript(
	topic: string,
	agentA: string,
	agentB: string
) {
	...
	while (attempts < 5) {
		...
		try {
			...
			const content = await generateRapTranscript(topic, agentA, agentB);
...
export async function generateRapTranscriptAudio({
	local,
	topic,
	agentA,
	agentB,
	...
	try {
		...
		let transcript = (await rapTranscript(
			topic,
			agentA,
			agentB
		)) as Transcript[];
